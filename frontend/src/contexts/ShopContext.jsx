...
export const ShopContextProvider = (props) => {
    const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://192.168.100.30:3000';
    const [products, setProducts] = useState([]);
    const [cart, setCart] = useState({ items: [], summary: { subtotal: 0, totalItems: 0 } });
    ...
    const addToCart = async (productId, variant = null, quantity = 1, deliveryDetails = null, categoryId = null, categorySlug = null) => {
        if (!token) {
    ...
    const addSubscriptionToCart = async (subscriptionId, variant, quantity = 1, deliveryDetails = null, name, price, variantLabel, duration) => {
        if (!token) {
            toast.error("Please login first");
            return { success: false };
        }
        ...
        try {
            ...
            const errorMsg = error.response?.data?.message || "Failed to add subscription to cart";
            toast.error(errorMsg);
            return { success: false, error: errorMsg };
        } finally {
            setLoading(false);
